...
require_once(dirname(__DIR__) . "/uesp-esolog/esoCommon.php");
require_once(dirname(__DIR__) . "/uesp-esolog/viewCps.class.php");
require_once(dirname(__DIR__) . "/uesp-esolog/viewSkills.class.php");
...
class EsoBuildDataEditor
{
	...
	public function GetCharacterEquippedItem($slotIndex) 
	{
		...
		if ($this->buildDataViewer->characterData === null) return null;
		if (!array_key_exists('equipSlots', $this->buildDataViewer->characterData)) return null;
	...
	public function CreateNewInitialSkillData()
	{
		...
		{
			$race = array_key_exists($skillId, $ESO_FREE_RACIAL_SKILLS)
				? $ESO_FREE_RACIAL_SKILLS[$skillId]
				: null;
	...
	public function LoadRulesFromDb()
	{
		...
		if (count($this->buildRules) === 0)
		{
			return $this->ReportError("No rows in MySQL table 'rules' for version '$safeVersion'. Import uesp-esochardata/seed.sql (or run generate_seed_sql.py and re-import) and confirm UESP_MYSQL_* points at that database.");
		}
		if (count($this->buildComputedStats) === 0)
		{
			return $this->ReportError("No rows in MySQL table 'computedStats' for version '$safeVersion'. Re-export rules.json with a full 'stats' section and regenerate seed.sql.");
		}
	...
	public function LoadBuild()
	{
		if ($this->buildId === null)
		{
			...
			{
				if (!$this->LoadRulesFromDb()) return false;
			}
			else
			{
				$this->rulesVersion = "";
	...
	public function GetEditorErrorBannerHtml()
	{
		$msgs = array_merge($this->errorMessages, $this->buildDataViewer->errorMessages);
		if (count($msgs) === 0) return "";
		$body = implode("<br/>", array_map(array('EsoBuildDataEditor', 'escapeHtml'), $msgs));
		return "<div class='esotbEditorErrorBanner' style='background:#ffe0e0;border:1px solid #c00;padding:8px;margin:8px 0;'><b>Build editor notice:</b><br/>$body</div>";
	}
	...
	{
		$replacePairs = array(
				'{editorErrorBanner}' => $this->GetEditorErrorBannerHtml(),
	...
	public function SetSessionData()
	{
		...
		// Standalone testBuild.php / php -S: no MediaWiki request object
		if (!isset($wgRequest) || $wgRequest === null) {
			return true;
		}
	...
	public function GetOutputHtml()
	{
		...
		if (!$this->LoadBuild())
		{
			$msg = ($this->buildId !== null)
				? "Could not find the specified character build! It was likely deleted by the owner and is no longer available."
				: "The build editor cannot load required rule data from MySQL (see details below).";
			return $this->OutputError($msg);
		}
