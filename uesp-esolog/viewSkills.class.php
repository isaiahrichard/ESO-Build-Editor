...
require_once(dirname(__DIR__) . "/secrets/esolog.secrets.php");
require_once(__DIR__ . "/esoCommon.php");
require_once(__DIR__ . "/esoRemoteMinedData.php");
...
class CEsoViewSkills
{
	...
	/**
	 * Parse memory_limit values like "128M", "512M", or byte integers from php.ini.
	 *
	 * @return int Byte count, or -1 for empty / unlimited (-1).
	 */
	private static function IniMemoryToBytes($value)
	{
	...
	/**
	 * minedSkills + full skillTooltips in arrays plus json_encode() for the embed routinely exceed 128M.
	 * Raises memory only when the current limit is finite and below 512M; does not lower unlimited (-1) or high caps.
	 */
	private static function EnsureSkillViewerMemoryLimit()
	{
		...
		$curBytes = self::IniMemoryToBytes($current);
		if ($curBytes < 0) {
			return;
		}
		$minBytes = 512 * 1024 * 1024;
	...
	private function LoadCraftedSkills()
	{
		...
		if (uesp_eso_remote_mined_data_enabled()) {
			$craftedSkillTable = "craftedSkills" . $this->GetTableSuffix();
			try {
				$result = $this->db->query("SELECT 1 FROM $craftedSkillTable LIMIT 1");
				if ($result === false) {
					$this->craftedSkills = array();
					$this->craftedScripts = array();
					return true;
				}
			} catch (Throwable $e) {
				$this->craftedSkills = array();
	...
	public function GetSkillTreeTypeHtml($skillType, $isClass)
	{
		...
		$typeTree = (is_array($this->skillTree) && isset($this->skillTree[$skillType]) && is_array($this->skillTree[$skillType]))
			? $this->skillTree[$skillType]
			: [];
		foreach ($typeTree as $skillLine => $skillLineData)
		{
			$index++;
			$isRaceVisible = true;
	...
	{
		if (!is_array($this->skills) || !array_key_exists($this->highlightSkillId, $this->skills)) {
			return false;
		}
	...
	public function GetUpdateDate()
	{
		...
		try {
			...
			if (!$row || !array_key_exists("value", $row)) return "";
			return $row["value"];
		} catch (Throwable $e) {
			return "";
		}
	...
	public function GetSubclassPopupClassHtml($className)
	{
		...
		$classTree = (is_array($this->skillTree) && isset($this->skillTree[$className]) && is_array($this->skillTree[$className]))
			? $this->skillTree[$className]
			: [];
		foreach ($classTree as $skillLine => $skillLineData)
		{
	...
	public function LoadData()
	{
		...
		if (defined('UESP_ESO_LOCAL_MINIMAL') && UESP_ESO_LOCAL_MINIMAL) {
			$this->LoadTemplate();
			$this->skills = array();
			$this->craftedSkills = array();
			$this->craftedScripts = array();
			...
			$this->hasSkillTooltips = false;
			$this->dataLoaded = true;
			return true;
		}

		self::EnsureSkillViewerMemoryLimit();

		$this->LoadTemplate();

		$localSkillError = null;
		$skillsOk = false;
		...
		if (!$skillsOk && !$this->LoadSkillsAndTooltipsFromRemoteExport()) {
			$hint = ' Apply uesp-esochardata/sql/mined_tables_schema.sql then run: php scripts/import-mined-data.php '
				. '(or place mined-export.json under data/uesp-export/). Enable extension=openssl for HTTPS.';

			if ($localSkillError !== null) {
				throw new RuntimeException(
					'EsoBuildEditor: Skill data unavailable. Local MySQL skill tables failed: ' . $localSkillError->getMessage()
					. ' — and no usable data from mined-export.json or exportJson.php.' . $hint,
					0,
					$localSkillError
				);
			}
			...
			throw new RuntimeException(
				'EsoBuildEditor: Skill data unavailable (MySQL load returned false).' . $hint
			);
		}
	...
	/**
	 * Same data as LoadSkills() + LoadSkillTooltips() but from UESP exportJson (no local mined tables).
	 */
	private function LoadSkillsAndTooltipsFromRemoteExport()
	{
		$data = uesp_eso_get_mined_export_json($this->version, array('minedSkills', 'skillTree', 'skillTooltips'));
		if ($data === null) {
			return false;
		}
		if (empty($data['skillTree']) || !is_array($data['skillTree'])) {
			return false;
		}
		if (empty($data['minedSkills']) || !is_array($data['minedSkills'])) {
			return false;
		}
		...
		$minedById = array();
		foreach ($data['minedSkills'] as $row) {
			$minedById[intval($row['id'])] = $row;
		}

		$this->skills = array();

		foreach ($data['skillTree'] as $treeRow) {
			$abilityId = intval($treeRow['abilityId']);
			$mined = isset($minedById[$abilityId]) ? $minedById[$abilityId] : array();
			$row = array_merge($mined, $treeRow);
		...
		foreach ($data['minedSkills'] as $row) {
			if ($row['setName'] === '' || $row['setName'] === null) {
				continue;
			}
			$id = intval($row['id']);
			$index = count($this->skills);
		...
		$this->LogProfile("LoadSkillsAndTooltipsFromRemoteExport(skills)", $startTime);
		...
		$this->skillTooltips = array();
		$this->hasSkillTooltips = false;
		if (!empty($data['skillTooltips']) && is_array($data['skillTooltips'])) {
			foreach ($data['skillTooltips'] as $row) {
				$abilityId = intval($row['abilityId']);
				$tooltipIndex = intval($row['idx']);
			...
			$this->hasSkillTooltips = count($this->skillTooltips) > 0;
		}
