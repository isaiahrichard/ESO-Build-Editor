...
class CEsoViewCP
{
	...
	private function LoadCp2Data()
	{
		...
		if ($this->LoadCp2DataFromRemoteExport()) {
			$this->CreateCp2LinksData();
			$this->CreateCp2UnlockData();
			$this->isDataLoaded = true;
			return true;
		...
		$hint = ' Apply uesp-esochardata/sql/mined_tables_schema.sql then run: php scripts/import-mined-data.php '
			. '(or save JSON from the browser to data/uesp-export/mined-export.json — see script header). '
			. 'Enable extension=openssl in php.ini for HTTPS from PHP.';

		if ($localError !== null) {
			throw new RuntimeException(
				'EsoBuildEditor: Champion Point data unavailable. Local MySQL cp2* load failed: ' . $localError->getMessage()
				. ' — and no usable data from data/uesp-export/mined-export.json or exportJson.php.' . $hint,
				0,
				$localError
			);
		}
		...
		throw new RuntimeException(
			'EsoBuildEditor: Champion Point data unavailable (MySQL load returned false).'
			. $hint
		);
	}
	...
	/**
	 * Empty CP v2 shell so templates do not read missing keys (offline / no mined tables / remote fetch failed).
	 */
	private function ApplyCp2V2MinimalStub()
	{
		...
		$this->cpData = array(
			1 => array('name' => 'Craft', 'description' => '', 'disciplineIndex' => 1, 'attribute' => '', 'skills' => array()),
			2 => array('name' => 'Warfare', 'description' => '', 'disciplineIndex' => 2, 'attribute' => '', 'skills' => array()),
			3 => array('name' => 'Fitness', 'description' => '', 'disciplineIndex' => 3, 'attribute' => '', 'skills' => array()),
		);
		...
		if (empty($this->initialData['slots']) || !is_array($this->initialData['slots'])) {
			$this->initialData['slots'] = array();
			for ($i = 1; $i <= 12; ++$i) {
				$this->initialData['slots'][$i] = null;
			}
		}
	}
	...
	/**
	 * Populate CP v2 structures from UESP exportJson (no local cp2* tables required).
	 */
	private function LoadCp2DataFromRemoteExport()
	{
		$data = uesp_eso_get_mined_export_json($this->version, array(
			'cp2Disciplines',
			'cp2ClusterRoots',
			'cp2SkillLinks',
			'cp2Skills',
		...
		if (empty($data['cp2Disciplines']) || !is_array($data['cp2Disciplines'])) {
			return false;
		}
		...
		foreach ($data['cp2Disciplines'] as $row) {
			$name = $row['name'];
		...
		$this->cpClusters = array();
		if (!empty($data['cp2ClusterRoots']) && is_array($data['cp2ClusterRoots'])) {
			foreach ($data['cp2ClusterRoots'] as $row) {
		...
		$this->cpLinks = array();
		if (!empty($data['cp2SkillLinks']) && is_array($data['cp2SkillLinks'])) {
			foreach ($data['cp2SkillLinks'] as $row) {
				$this->cpLinks[] = $row;
			}
		}
		...
		if (empty($data['cp2Skills']) || !is_array($data['cp2Skills'])) {
			return false;
		}

		foreach ($data['cp2Skills'] as $row) {
		...
		foreach ($this->cpData as $discIndex => &$cpData) {
			uasort($this->cpData[$discIndex]['skills'], array('CEsoViewCP', 'sortSkillsName'));
		}
		...
		if (!empty($data['cp2SkillDescriptions']) && is_array($data['cp2SkillDescriptions'])) {
			foreach ($data['cp2SkillDescriptions'] as $row) {
				...
				if (!array_key_exists($abilityId, $this->cpSkillDesc) || $this->cpSkillDesc[$abilityId] == null) {
					$this->cpSkillDesc[$abilityId] = array();
				}
	...
	public function GetInitialClusterValueV2($clusterId)
	{
		...
		$cluster = is_array($this->cpClusters) ? ($this->cpClusters[$clusterId] ?? null) : null;
		if ($cluster == null) return 0;
		
		$discIndex = $cluster['disciplineIndex'];
		if (empty($this->cpData[$discIndex]) || !is_array($this->cpData[$discIndex])) return 0;
		$discName = $this->cpData[$discIndex]['name'];
		if ($discName == null) return 0;
		
		$totalPoints = 0;
		
		$skills = $this->cpData[$discIndex]['skills'] ?? [];
		if (!is_array($skills)) return 0;
		
		foreach ($skills as $abilityId => $skill)
		{
	...
	{
		if (!isset($this->cpData[$discIndex]) || !is_array($this->cpData[$discIndex]) || !isset($this->cpData[$discIndex]['name'])) {
			return "";
		}
		if (!is_array($this->initialData) || !isset($this->initialData['slots']) || !is_array($this->initialData['slots'])) {
			return "";
		}
